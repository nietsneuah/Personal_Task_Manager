...
const FilterBar: React.FC<FilterBarProps> = ({ className = '' }) => {
  ...
  const resetFilters = () => {
    ...
    // Clear global filters and force update
    setFilters({});
    
    try {
      // Force immediate application of the reset
      // Use a small delay to ensure state updates have been processed
      setTimeout(() => {
        // This ensures the table view is refreshed
        const resetEvent = new CustomEvent('filter-reset');
        window.dispatchEvent(resetEvent);
        console.log('Filter reset event dispatched');
      }, 50);
    } catch (error) {
      console.error('Error dispatching filter reset event:', error);
    }
  };
  ...
  return (
    <div className={`bg-gray-800 p-4 rounded-lg shadow-sm ${className}`}>
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        {/* Priority Filter */}
        <div className="flex flex-col">
          <label className="text-sm font-medium text-gray-300 mb-1">Priority</label>
          <div className="flex space-x-2">
        ...
        {/* Impact Filter */}
        <div className="flex flex-col">
          <label className="text-sm font-medium text-gray-300 mb-1">Impact</label>
          <div className="flex space-x-2">
        ...
        {/* Urgency Filter */}
        <div className="flex flex-col">
          <label className="text-sm font-medium text-gray-300 mb-1">Urgency</label>
          <div className="flex space-x-2">
        ...
        {/* Due Date Filter */}
        <div className="flex flex-col">
          <label className="text-sm font-medium text-gray-300 mb-1">Due Date</label>
          <input
            type="date"
            value={dueDateFilter}
            onChange={e => setDueDateFilter(e.target.value)}
            className="px-3 py-1 bg-gray-700 border border-gray-600 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        ...
        <div className="mt-4 p-4 bg-gray-700 rounded-md">
          <div className="flex items-center space-x-2">
            <input
              ...
              className="flex-grow px-3 py-2 bg-gray-800 border border-gray-600 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <button
              onClick={savePreset}
              className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors"
      ...
      {presets.length > 0 && (
        <div className="mt-4">
          <h3 className="text-sm font-medium text-gray-300 mb-2">Saved Presets</h3>
          <div className="flex flex-wrap gap-2">
            {presets.map((preset, index) => (
              <div key={index} className="flex items-center bg-gray-700 rounded-md overflow-hidden">
                <button
                  onClick={() => loadPreset(preset)}
                  className="px-3 py-1 text-sm text-gray-300 hover:bg-gray-600"
                >
                  {preset.name}
                </button>
                <button
